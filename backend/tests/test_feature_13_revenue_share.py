...
def test_revenue_share_is_fixed_funded_snapshot_and_executes_once():
    with TestClient(app) as client:
        ...
        dist=client.post('/api/revenue-share',headers=creator,json={'campaign_id':cid,'title':'Realised community pool','asset_symbol':'REV13','funded_amount':100,'funding_reference':'feature13-real-pool'})
        assert dist.status_code==200 and dist.json()['status']=='PENDING'
        did=dist.json()['id']
        assert 'not a promised yield' in dist.json()['disclaimer'].lower()
        assert client.post(f'/api/revenue-share/{did}/activate',headers=admin).status_code==200
