...
def test_revenue_share_is_fixed_funded_snapshot_and_executes_once():
    with TestClient(app) as client:
        ...
        project=client.post('/api/projects',headers=creator,json={'name':'Feature Thirteen Revenue','symbol':'BASE13','description':'An isolated project used to prove fixed funded community revenue distributions.','chain':'Robinhood'})
        ...
        campaign=client.post('/api/campaigns',headers=creator,json={
            'project_id':pid,'title':'Qualifying Contributor Bag','description':'Complete this funded Challenge Bag to enter the later fixed distribution snapshot.',
            ...
            'missions':[],'challenges':[{'title':'Qualify','description':'Complete the contributor qualification quiz.','category':'LEARN','verification_type':'QUIZ','config':{'answer':'qualify'},'xp_reward':10}]
        })
        ...
        for headers in (u1,u2):
            ...
            done=client.post(f"/api/challenges/{challenge['id']}/complete",headers=headers,json={'answer':'qualify'})
