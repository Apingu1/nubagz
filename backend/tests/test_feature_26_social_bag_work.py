...
def test_unified_bag_work_requires_join_verifies_free_x_proof_and_settles_once():
    with TestClient(app) as client:
        ...
        campaign=client.post('/api/campaigns',headers=creator,json={'project_id':pid,'title':'Unified Bag Work test','description':'One funded Bag containing social and quiz work in the same challenge model.','category':'DISCOVER','difficulty':'EASY','reward_asset':'BW26','funding_type':'TOKEN','token_allocation':20,'gross_reward_per_user':20,'user_share_pct':80,'nubagz_share_pct':15,'referral_share_pct':5,'max_users':1,'challenges':[{'title':'Post about the launch','description':'Publish a public X post about the launch.','category':'SOCIAL','provider':'X','action':'POST','verification_type':'AUTO','config':{'required_text':'NuBagz launch'},'xp_reward':25},{'title':'Confirm the project brief','description':'Answer a simple quiz after reading the project brief.','category':'LEARN','verification_type':'QUIZ','config':{'question':'What is the project token?','options':['BW26','BTC'],'answer':'BW26'},'xp_reward':25}]});assert campaign.status_code==200,campaign.text;cid=campaign.json()['id'];assert len(campaign.json()['challenges'])==2;assert campaign.json()['challenges'][0]['action']=='POST';assert client.post(f'/api/funding/campaigns/{cid}/verify',headers=admin,json={'amount':20,'tx_hash':'0xfeature26fund'}).status_code==200;assert client.patch(f'/api/admin/campaigns/{cid}',headers=admin,json={'status':'LIVE'}).status_code==200
        db=SessionLocal()
        ...
        feed=client.get('/api/challenges',headers=worker);assert feed.status_code==200;items=[r for r in feed.json() if r['campaign_id']==cid];assert len(items)==2;social=next(r for r in items if r['category']=='SOCIAL');quiz=next(r for r in items if r['verification_type']=='QUIZ');assert social['proof_code'].startswith('NBZ-');assert social['config']['required_text']=='NuBagz launch'
        blocked=client.post(f"/api/challenges/{social['id']}/complete",headers=worker,json={'evidence':'https://x.com/feature26worker/status/123456789'});assert blocked.status_code==400 and 'join this bag' in blocked.json()['detail'].lower()
...
def test_paid_style_x_actions_and_self_attest_are_not_accepted_for_new_bag_work():
    ...
    try: ChallengeCreate(title='One click completion',category='BAG_WORK',verification_type='SELF_ATTEST',xp_reward=1)
    except ValueError as exc: assert 'automatic, quiz-based, or project-reviewed evidence' in str(exc).lower()
    else: raise AssertionError('SELF_ATTEST should not be accepted for new Bag Work')
