...
def test_campaign_templates_reuse_structure_without_bypassing_funding_activation_or_onchain_rules():
    with TestClient(app) as client:
        ...
        # Instantiating the creator template preserves the rule but never carries funding/approval.
        ...
        clone=cloned.json();assert clone['status']=='PENDING' and clone['funding_status']=='UNFUNDED' and clone['onchain_rules_created']==1
        ...
        assert client.patch(f"/api/admin/campaigns/{clone['id']}",headers=admin,json={'status':'LIVE'}).status_code==400
