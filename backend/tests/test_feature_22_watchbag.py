...
def test_watchbag_only_saves_live_funded_challenge_bagz_and_is_idempotent():
    with TestClient(app) as client:
        ...
        project=client.post('/api/projects',headers=creator,json={'name':'Feature Twenty Two WatchBag','symbol':'WTCH22','description':'An isolated Robinhood project used to prove WatchBag saves only live funded public opportunities.','chain':'Robinhood'});assert project.status_code==200
        pid=project.json()['id'];assert client.patch(f'/api/admin/projects/{pid}',headers=admin,json={'status':'APPROVED'}).status_code==200
        pending=client.post('/api/campaigns',headers=creator,json={'project_id':pid,'title':'Pending Watch Attempt','description':'This Challenge Bag remains pending and must not be watchable before verified funding and activation.','category':'LEARN','difficulty':'EASY','reward_asset':'WTCH22','funding_type':'TOKEN','token_allocation':10,'gross_reward_per_user':10,'user_share_pct':80,'nubagz_share_pct':15,'referral_share_pct':5,'max_users':1,'missions':[],'challenges':[{'title':'Pending proof','description':'A deterministic quiz Challenge for WatchBag lifecycle testing.','category':'LEARN','verification_type':'QUIZ','config':{'answer':'watch'},'xp_reward':10}]});assert pending.status_code==200
