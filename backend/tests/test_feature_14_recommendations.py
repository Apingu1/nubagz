...
def test_recommendations_are_explainable_sorted_and_never_override_access():
    with TestClient(app) as client:
        ...
        assert client.post(f"/api/access/campaigns/{target['id']}",headers=creator,json={'min_bag_score':1000}).status_code==200
        ...
        assert client.post(f"/api/access/campaigns/{target['id']}",headers=creator,json={'min_bag_score':0}).status_code==200
        restored=client.get('/api/recommendations/me',headers=earner)
        assert restored.status_code==200
        assert any(r['campaign_id']==target['id'] for r in restored.json()['recommendations'])
