...
def test_report_is_a_private_due_process_case_not_an_automatic_or_repeatable_penalty():
    with TestClient(app) as client:
        ...
        duplicate=client.post('/api/reports',headers=reporter,json={'target_type':'PROJECT','target_id':pid,'category':'SAFETY','detail':'A duplicate open case for the same target and category should be rejected instead of creating moderation spam.'})
        assert duplicate.status_code==409

        assert client.get(f'/api/reports/{rid}',headers=outsider).status_code==404
        reporter_message=client.post(f'/api/reports/{rid}/messages',headers=reporter,json={'message':'Additional reporter evidence for the moderator and affected project owner.'})
        assert reporter_message.status_code==200
        ...
        assert found['messages'][0]['author']=='Participant'
        assert 'Feature19Reporter' not in str(found)
        ...
        # Dismissal is a no-penalty outcome: it cannot be paired with a suspension action.
        ...
        pid2=project2.json()['id'];assert client.patch(f'/api/admin/projects/{pid2}',headers=admin,json={'status':'APPROVED'}).status_code==200
        opened2=client.post('/api/reports',headers=reporter,json={'target_type':'PROJECT','target_id':pid2,'category':'OTHER','detail':'A second moderation case used to prove a dismissed allegation leaves the target unchanged.'});assert opened2.status_code==200
        rid2=opened2.json()['id']
        bad_dismiss=client.post(f'/api/reports/{rid2}/resolve',headers=admin,json={'status':'DISMISSED','action':'SUSPEND_PROJECT','note':'This combination should be rejected.'})
        assert bad_dismiss.status_code==400
        dismissed=client.post(f'/api/reports/{rid2}/resolve',headers=admin,json={'status':'DISMISSED','action':'NONE','note':'Reviewed and dismissed without a moderation penalty.'})
        assert dismissed.status_code==200 and dismissed.json()['resolution_action']=='NONE'
        assert project_status(client,admin,pid2)=='APPROVED'
