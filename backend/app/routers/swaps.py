...
def serialize(row: SwapIntent, wallet: WalletConnection):
    return {
        "id": row.id, "chain": row.chain, "sell_asset": row.sell_asset, "buy_asset": row.buy_asset,
        "sell_amount": str(row.sell_amount), "max_slippage_bps": row.max_slippage_bps, "status": row.status,
        "wallet_address": wallet.address, "provider": row.provider_name, "provider_quote_id": row.provider_quote_id,
...
def create_intent(data: SwapIntentIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    sell_asset = data.sell_asset.strip().upper(); buy_asset = data.buy_asset.strip().upper()
    if sell_asset == buy_asset:
        raise HTTPException(400, "Sell and buy assets must be different")
    wallet = verified_wallet(db, user)
    row = SwapIntent(user_id=user.id, wallet_connection_id=wallet.id, chain=chain.title(), sell_asset=sell_asset, buy_asset=buy_asset, sell_amount=data.sell_amount, max_slippage_bps=data.max_slippage_bps)
...
def quote_intent(intent_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    request_payload = {
        "wallet_address": wallet.address, "chain": row.chain, "sell_asset": row.sell_asset, "buy_asset": row.buy_asset,
        "sell_amount": str(row.sell_amount), "max_slippage_bps": row.max_slippage_bps,
    }
    try:
        response = httpx.post(url, json=request_payload, headers=headers, timeout=12.0)
        response.raise_for_status(); payload = response.json()
    except Exception as exc:
        raise HTTPException(502, "Swap provider quote request failed") from exc
    quote_id = payload.get("quote_id"); buy_amount = payload.get("buy_amount"); transaction = payload.get("transaction")
    ...
    row.status = "QUOTED"; row.provider_name = payload.get("provider") or "configured-provider"; row.provider_quote_id = str(quote_id); row.quoted_buy_amount = quoted_buy; row.quote_expires_at = expires_at; row.transaction_payload = json.dumps(transaction, separators=(",", ":"))
