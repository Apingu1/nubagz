...
def serialize_bounty(row: Bounty, db: Session, user_id: int | None = None):
    project = db.get(Project, row.project_id)
    mine = None
    if user_id:
        mine = db.query(BountySubmission).filter(BountySubmission.bounty_id == row.id, BountySubmission.user_id == user_id).first()
    return {
        "id": row.id, "project_id": row.project_id, "project_name": project.name if project else None,
        "title": row.title, "description": row.description, "reward_asset": row.reward_asset,
        "reward_per_winner": str(row.reward_per_winner), "max_winners": row.max_winners,
        "winners_count": row.winners_count, "remaining_winners": max(0, row.max_winners-row.winners_count),
        "funded_amount": str(row.funded_amount), "distributed_amount": str(row.distributed_amount),
        "funding_status": row.funding_status, "status": row.status,
        "my_submission_status": mine.status if mine else None, "created_at": row.created_at.isoformat(),
    }
...
def serialize_submission(row: BountySubmission, db: Session):
    ...
    return {"id":row.id,"bounty_id":row.bounty_id,"bounty_title":bounty.title if bounty else None,"user_id":row.user_id,"username":user.username if user else None,"evidence":row.evidence,"status":row.status,"created_at":row.created_at.isoformat()}


@router.get("")
def live_bounties(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    rows = db.query(Bounty).filter(Bounty.status == "LIVE", Bounty.funding_status == "VERIFIED").order_by(Bounty.created_at.desc()).all()
...
def my_bounties(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    rows = db.query(Bounty).filter(Bounty.created_by_id == user.id).order_by(Bounty.created_at.desc()).all()
    return [serialize_bounty(row, db, user.id) for row in rows]
...
def review_queue(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    rows = db.query(BountySubmission).join(Bounty, Bounty.id == BountySubmission.bounty_id).join(Project, Project.id == Bounty.project_id).filter(Project.owner_id == user.id).order_by(BountySubmission.created_at.desc()).all()
...
def create_bounty(data: BountyIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    row = Bounty(project_id=project.id, created_by_id=user.id, title=data.title, description=data.description, reward_asset=data.reward_asset.upper(), reward_per_winner=data.reward_per_winner, max_winners=data.max_winners, funded_amount=data.funded_amount, funding_reference=data.funding_reference)
    db.add(row); db.commit(); db.refresh(row)
    return serialize_bounty(row, db, user.id)


@router.post("/{bounty_id}/activate")
def activate_bounty(bounty_id: int, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    row = db.get(Bounty, bounty_id)
    if not row:
        raise HTTPException(404, "Bounty not found")
    required = Decimal(row.reward_per_winner) * Decimal(row.max_winners)
    if not row.funding_reference or Decimal(row.funded_amount) < required:
        raise HTTPException(400, "Bounty is not fully funded")
    row.funding_status = "VERIFIED"; row.status = "LIVE"; db.commit()
    return serialize_bounty(row, db)
...
def submit_bounty(bounty_id: int, data: SubmissionIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if db.query(BountySubmission).filter(BountySubmission.bounty_id == row.id, BountySubmission.user_id == user.id).first():
        raise HTTPException(409, "You already submitted to this bounty")
    submission = BountySubmission(bounty_id=row.id, user_id=user.id, evidence=data.evidence)
...
def decide_submission(submission_id: int, data: DecisionIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    submission = db.query(BountySubmission).filter(BountySubmission.id == submission_id).with_for_update().first()
    bounty = db.query(Bounty).filter(Bounty.id == submission.bounty_id).with_for_update().first() if submission else None
    ...
    if status == "APPROVED":
        participant = db.get(User, submission.user_id)
        trust = db.query(UserTrustProfile).filter(UserTrustProfile.user_id == submission.user_id).first()
        if trust and trust.trust_level == "RESTRICTED":
            raise HTTPException(409, "Submission cannot be rewarded while the user is Restricted")
        if not participant or bounty.status != "LIVE" or bounty.funding_status != "VERIFIED":
        ...
        bounty.winners_count += 1; bounty.distributed_amount = Decimal(bounty.distributed_amount) + reward
        db.add(LedgerEntry(user_id=submission.user_id, campaign_id=None, asset_symbol=bounty.reward_asset, amount=reward, entry_type="BOUNTY_REWARD", note=f"Approved bounty: {bounty.title}"))
        if bounty.winners_count >= bounty.max_winners:
            bounty.status = "COMPLETED"
    submission.status = status; submission.reviewed_by_id = user.id; submission.reviewed_at = datetime.now(UTC)
    db.commit(); return serialize_submission(submission, db)
