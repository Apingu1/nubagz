...
def summary(project:Project,db:Session,user:User):
    rows=db.query(ProjectReview).filter(ProjectReview.project_id==project.id,ProjectReview.status=="PUBLISHED").order_by(ProjectReview.updated_at.desc()).all();avg=(sum(Decimal(row.rating) for row in rows)/Decimal(len(rows))) if rows else Decimal("0")
    mine_any=db.query(ProjectReview).filter(ProjectReview.project_id==project.id,ProjectReview.user_id==user.id).first();mine=mine_any if mine_any and mine_any.status=="PUBLISHED" else None;hidden_by_moderation=bool(mine_any and mine_any.status=="HIDDEN")
...
def project_reviews(project_id:int,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    if not project or project.status not in PUBLIC_PROJECT_STATUSES:raise HTTPException(404,"Live project not found")
...
def write_review(project_id:int,data:ReviewIn,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    project=db.get(Project,project_id)
    if not project or project.status not in PUBLIC_PROJECT_STATUSES:raise HTTPException(404,"Live project not found")
    if project.owner_id==user.id:raise HTTPException(403,"Project owners cannot review their own project")
    if not eligible(db,user,project):raise HTTPException(403,"Complete a funded campaign for this project before reviewing your participant experience")
    row=db.query(ProjectReview).filter(ProjectReview.project_id==project.id,ProjectReview.user_id==user.id).first()
    if row and row.status=="HIDDEN":raise HTTPException(409,"This review was hidden by moderation and cannot be republished through a normal edit")
    if not row:
        row=ProjectReview(project_id=project.id,user_id=user.id,rating=data.rating,review=data.review);db.add(row)
    else:
        row.rating=data.rating;row.review=data.review
    db.commit();db.refresh(row);return review_payload(row,db)
