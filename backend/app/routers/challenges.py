...
from ..x_verifier import XVerificationUnavailable, verify_x_action
...
def _settle_campaign(db: Session, user: User, campaign: Campaign, enrollment: Enrollment) -> None:
    ...
    if builder_id and builder_amount > 0:
        db.add(LedgerEntry(
            ...
            entry_type="BUILDER_SHARE",
            note=f"BagBuilder share from {campaign.title}",
        ))
    db.add(LedgerEntry(
        user_id=None,
        campaign_id=campaign.id,
...
def _serialize_feed_row(challenge: Challenge, campaign: Campaign, project: Project, completion: ChallengeCompletion | None) -> dict:
...
def list_bag_work(
    ...
    for challenge, campaign, project in q.order_by(Campaign.featured.desc(), Campaign.created_at.desc(), Challenge.position).all():
        ...
        rows.append(_serialize_feed_row(challenge, campaign, project, completion))
    return rows
...
def complete_challenge(
    ...
    completion = ChallengeCompletion(
        ...
        status="PENDING",
        answer=data.answer,
        evidence={"submission": data.evidence} if data.evidence else None,
    )
    db.add(completion)
    db.flush()
    ...
    if verification == "QUIZ":
        ...
        if challenge.category != "SOCIAL" or challenge.provider != "X":
        ...
        try:
            verified, evidence = verify_x_action(account, challenge)
        except XVerificationUnavailable as exc:
            raise HTTPException(503, str(exc)) from exc
        if not verified:
            raise HTTPException(400, "X has not confirmed this activity yet. Complete it on X and try again.")
