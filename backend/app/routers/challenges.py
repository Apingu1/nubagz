...
def _ensure_enrollment(db: Session, user: User, campaign: Campaign) -> Enrollment:
    ...
    enrollment = Enrollment(user_id=user.id, campaign_id=campaign.id); db.add(enrollment); db.flush(); return enrollment
...
def _settle_campaign(db: Session, user: User, campaign: Campaign, enrollment: Enrollment) -> None:
    ...
    if not _funding_available(db, campaign, gross): raise HTTPException(409, "Reward inventory was exhausted before this Bag could settle")
    ...
    enrollment.status = "COMPLETED"; enrollment.completed_at = datetime.now(UTC); enrollment.earned_amount = user_amount
...
def _finalize_completion(db: Session,user: User,campaign: Campaign,challenge: Challenge,completion: ChallengeCompletion,status: str,evidence: dict | None = None) -> bool:
    ...
    if will_complete and not _funding_available(db, campaign, Decimal(campaign.gross_reward_per_user)): raise HTTPException(409, "Reward inventory was exhausted before this Bag could settle")
    now = datetime.now(UTC); completion.status = status; completion.evidence = evidence or completion.evidence; completion.verified_at = now; completion.completed_at = now
    enrollment.completed_count = verified_total; user.xp += challenge.xp_reward; user.bag_score = min(1000, user.bag_score + max(1, challenge.xp_reward // 10))
    if will_complete: _settle_campaign(db, user, campaign, enrollment)
    return will_complete


def _public_config(challenge: Challenge) -> dict:
    config = dict(challenge.config or {}); config.pop("answer", None); return config


def _gas_summary(db: Session, challenge: Challenge) -> dict | None:
    if challenge.category != "ONCHAIN": return None
    policy = db.query(GasSponsorshipPolicy).filter(GasSponsorshipPolicy.challenge_id == challenge.id).first()
    if not policy: return {"enabled": False, "status": "USER_PAID"}
    now = datetime.now(UTC)
    active = policy.status == "ACTIVE" and policy.funding_status == "VERIFIED" and (not policy.starts_at or policy.starts_at <= now) and (not policy.ends_at or policy.ends_at >= now) and Decimal(policy.spent_amount) < Decimal(policy.funded_amount)
    return {
        "enabled": True, "active": active, "status": policy.status,
        "chain": policy.chain, "native_asset": policy.native_asset,
        "max_native_per_claim": str(policy.max_native_per_claim),
        "max_unique_users": policy.max_unique_users, "max_claims": policy.max_claims,
        "max_claims_per_wallet": policy.max_claims_per_wallet,
        "starts_at": policy.starts_at.isoformat() if policy.starts_at else None,
        "ends_at": policy.ends_at.isoformat() if policy.ends_at else None,
    }
...
def list_bag_work(category: str | None = Query(default=None),provider: str | None = Query(default=None),db: Session = Depends(get_db),user: User = Depends(get_current_user)):
    now = datetime.now(UTC)
    q = db.query(Challenge, Campaign, Project).join(Campaign, Campaign.id == Challenge.campaign_id).join(Project, Project.id == Campaign.project_id).filter(Challenge.status == "ACTIVE",Campaign.status == "LIVE",Project.status.notin_(["SUSPENDED","ARCHIVED"]))
...
def complete_challenge(challenge_id: int,data: ChallengeCompleteIn,db: Session = Depends(get_db),user: User = Depends(get_current_user)):
    ...
    db.add(completion); db.flush(); verification = challenge.verification_type.upper()
