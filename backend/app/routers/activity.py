...
def visible_project(project: Project | None) -> bool:
    return bool(project and project.status == "APPROVED")
...
def activity_feed(
    ...
    if requested in {None, "BAG_COMPLETED"}:
        ...
        for enrollment in completions:
            ...
            if not visible_user(user) or not visible_project(project) or not campaign or campaign.status not in {"LIVE", "COMPLETED"}:
                continue
            events.append({
                "event_id":f"completion:{enrollment.id}","event_type":"BAG_COMPLETED","username":user.username,
    ...
    if requested in {None, "BAGDROP_CLAIMED"}:
        ...
        for claim in claims:
            ...
            if not visible_user(user) or not visible_project(project) or not drop or drop.funding_status != "VERIFIED" or drop.status not in {"LIVE", "COMPLETED"}:
                continue
            events.append({
                "event_id":f"bagdrop:{claim.id}","event_type":"BAGDROP_CLAIMED","username":user.username,
    ...
    if requested in {None, "PROJECT_REVIEWED"}:
        ...
        for review in reviews:
            ...
            if not visible_user(user) or not visible_project(project):
                continue
            events.append({
                "event_id":f"review:{review.id}","event_type":"PROJECT_REVIEWED","username":user.username,
