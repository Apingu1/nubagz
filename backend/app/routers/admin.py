...
def overview(db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    return {"users":db.query(func.count(User.id)).scalar() or 0,"projects":db.query(func.count(Project.id)).scalar() or 0,"live_projects":db.query(func.count(Project.id)).filter(Project.status.in_(["LIVE","APPROVED"])).scalar() or 0,"suspended_projects":db.query(func.count(Project.id)).filter(Project.status == "SUSPENDED").scalar() or 0,"live_campaigns":db.query(func.count(Campaign.id)).filter(Campaign.status == "LIVE").scalar() or 0,"suspended_campaigns":db.query(func.count(Campaign.id)).filter(Campaign.status == "SUSPENDED").scalar() or 0,"completions":db.query(func.count(Enrollment.id)).filter(Enrollment.status == "COMPLETED").scalar() or 0,"pending_withdrawals":db.query(func.count(Withdrawal.id)).filter(Withdrawal.status == "PENDING").scalar() or 0,"active_gas_passes":db.query(func.count(GasSponsorshipPolicy.id)).filter(GasSponsorshipPolicy.status == "ACTIVE").scalar() or 0,"gas_funding_pending":db.query(func.count(GasSponsorshipPolicy.id)).filter(GasSponsorshipPolicy.funding_status != "VERIFIED").scalar() or 0,"open_flags":int(legacy_flags)+int(risk_signals)}
...
def moderate_project(project_id:int,data:AdminDecision,db:Session=Depends(get_db),_:User=Depends(require_admin)):
    ...
    requested=data.status.upper()
    # Compatibility only: old clients/tests may still send APPROVED/REJECTED.
    # The current UI never exposes an approval queue; these aliases map directly
    # to the new moderation states.
    status={"APPROVED":"LIVE","REJECTED":"SUSPENDED"}.get(requested,requested)
...
def campaigns(db:Session=Depends(get_db),_:User=Depends(require_admin)):
    rows=db.query(Campaign).order_by(Campaign.created_at.desc()).all();funding_rows={f.campaign_id:f for f in db.query(CampaignFunding).all()};payload=[]
    for c in rows:
