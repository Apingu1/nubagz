...
def serialize(row: RevenueShareDistribution, db: Session, user_id: int | None = None, include_funding_details: bool = False):
    ...
    recipient = db.query(RevenueShareRecipient).filter(
        RevenueShareRecipient.distribution_id == row.id,
        RevenueShareRecipient.user_id == user_id,
...
def distributions(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    rows = db.query(RevenueShareDistribution).filter(
        RevenueShareDistribution.funding_status == "VERIFIED"
    ).order_by(RevenueShareDistribution.created_at.desc()).all()
...
def my_distributions(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    rows = db.query(RevenueShareDistribution).filter(
        RevenueShareDistribution.created_by_id == user.id
    ).order_by(RevenueShareDistribution.created_at.desc()).all()
...
def create_distribution(data: DistributionIn, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    db.commit()
    db.refresh(row)
    return serialize(row, db, user.id, include_funding_details=True)
...
def execute_distribution(distribution_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    existing = db.query(RevenueShareRecipient).filter(
        RevenueShareRecipient.distribution_id == row.id
    ).first()
    if existing:
    ...
    for enrollment in completed:
        ...
        trust = db.query(UserTrustProfile).filter(
            UserTrustProfile.user_id == enrollment.user_id
        ).first()
    ...
    per_user = (Decimal(row.funded_amount) / Decimal(len(eligible_user_ids))).quantize(
    ...
    for user_id in eligible_user_ids:
        ...
        db.add(LedgerEntry(
            ...
            note=f"Funded community distribution: {row.title}",
        ))
    row.recipient_count = len(eligible_user_ids)
    row.amount_per_recipient = per_user
    row.distributed_amount = distributed
    row.status = "EXECUTED"
