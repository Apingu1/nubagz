...
def dashboard(db:Session=Depends(get_db), user:User=Depends(get_current_user)):
    ...
    return DashboardOut(lifetime_assets=len(balances),active_bagz=active,completed_bagz=completed,xp=user.xp,bag_score=user.bag_score,streak_days=user.streak_days,balances=balances,recent_activity=[{"asset":r.asset_symbol,"amount":str(r.amount),"type":r.entry_type,"note":r.note,"created_at":r.created_at.isoformat()} for r in recent])
...
def wallets(db:Session=Depends(get_db), user:User=Depends(get_current_user)):
    return db.query(WalletConnection).filter(WalletConnection.user_id==user.id).order_by(WalletConnection.is_primary.desc(),WalletConnection.last_connected_at.desc()).all()
...
def verify_wallet(data:WalletVerifyIn, db:Session=Depends(get_db), user:User=Depends(get_current_user)):
    ...
    if not wallet: wallet=WalletConnection(user_id=user.id,address=data.address); db.add(wallet)
    ...
    existing_count=db.query(func.count(WalletConnection.id)).filter(WalletConnection.user_id==user.id).scalar() or 0
    if data.make_primary or existing_count==0 or not user.wallet_address:
        db.query(WalletConnection).filter(WalletConnection.user_id==user.id).update({WalletConnection.is_primary:False}); db.query(PayoutAddress).filter(PayoutAddress.user_id==user.id).update({PayoutAddress.is_primary:False}); wallet.is_primary=True; user.wallet_address=data.address; user.wallet_chain="EVM"
...
def make_wallet_primary(wallet_id:int, db:Session=Depends(get_db), user:User=Depends(get_current_user)):
    ...
    db.query(WalletConnection).filter(WalletConnection.user_id==user.id).update({WalletConnection.is_primary:False}); db.query(PayoutAddress).filter(PayoutAddress.user_id==user.id).update({PayoutAddress.is_primary:False}); wallet.is_primary=True; user.wallet_address=wallet.address; user.wallet_chain="EVM"; db.commit()
    return {"ok":True}
