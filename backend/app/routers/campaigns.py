...
def serialize_campaign(c:Campaign,db:Session)->CampaignOut:
    enrolled=db.query(func.count(Enrollment.id)).filter(Enrollment.campaign_id==c.id).scalar() or 0;completed=db.query(func.count(Enrollment.id)).filter(Enrollment.campaign_id==c.id,Enrollment.status=="COMPLETED").scalar() or 0
    payload=CampaignOut.model_validate(c);payload.enrolled_count=enrolled;payload.completed_count=completed;payload.challenges=[]
    for row in db.query(Challenge).filter(Challenge.campaign_id==c.id).order_by(Challenge.position).all():
        public={"id":row.id,"campaign_id":row.campaign_id,"title":row.title,"description":row.description,"category":row.category,"provider":row.provider,"action":row.action,"verification_type":row.verification_type,"target_url":row.target_url,"target_id":row.target_id,"config":{k:v for k,v in (row.config or {}).items() if k!="answer"},"xp_reward":row.xp_reward,"position":row.position,"status":row.status,"created_at":row.created_at};payload.challenges.append(ChallengeOut.model_validate(public))
    return payload
...
def fully_funded(db:Session,campaign:Campaign)->bool:
    funding=db.query(CampaignFunding).filter(CampaignFunding.campaign_id==campaign.id).first();required=Decimal(campaign.gross_reward_per_user)*Decimal(campaign.max_users)
...
def list_campaigns(category:str|None=Query(default=None),featured:bool|None=Query(default=None),db:Session=Depends(get_db)):
    q=db.query(Campaign).join(Project).options(joinedload(Campaign.project),joinedload(Campaign.missions)).filter(Campaign.status=="LIVE",Project.status.in_(PUBLIC_PROJECT_STATUSES))
...
def my_campaigns(db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    rows=db.query(Campaign).join(Project).options(joinedload(Campaign.project),joinedload(Campaign.missions)).filter(Project.owner_id==user.id).order_by(Campaign.created_at.desc()).all();return [serialize_campaign(c,db) for c in rows]
...
def create_campaign(data:CampaignCreate,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    campaign=Campaign(**data.model_dump(exclude={"missions","challenges"}),status="DRAFT");db.add(campaign);db.flush()
    ...
    for idx,challenge_data in enumerate(data.challenges):
        challenge=Challenge(campaign_id=campaign.id,position=idx,**challenge_data.model_dump(exclude={"gas_sponsorship"}));db.add(challenge);db.flush();gas=challenge_data.gas_sponsorship
        if gas and gas.enabled:
            ...
            db.add(GasSponsorshipPolicy(challenge_id=challenge.id,project_id=project.id,created_by_id=user.id,chain=gas.chain.strip().title(),native_asset=GAS_NATIVE[chain],max_native_per_claim=gas.max_native_per_claim,max_unique_users=gas.max_unique_users,max_claims=gas.max_claims,max_claims_per_wallet=gas.max_claims_per_wallet,funded_amount=gas.funded_amount,funding_reference=gas.funding_reference.strip(),starts_at=gas.starts_at,ends_at=gas.ends_at,funding_status="DECLARED",status="FUNDING_PENDING"))
    db.commit();campaign=db.query(Campaign).options(joinedload(Campaign.project),joinedload(Campaign.missions)).filter(Campaign.id==campaign.id).first();return serialize_campaign(campaign,db)
...
def publish_campaign(campaign_id:int,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    if not fully_funded(db,campaign):
        required=Decimal(campaign.gross_reward_per_user)*Decimal(campaign.max_users);raise HTTPException(409,f"Verify at least {required} {campaign.reward_asset} of reward funding before publishing")
    active_work=db.query(func.count(Challenge.id)).filter(Challenge.campaign_id==campaign.id,Challenge.status=="ACTIVE").scalar() or 0;legacy_work=db.query(func.count(Mission.id)).filter(Mission.campaign_id==campaign.id).scalar() or 0
...
def get_campaign(campaign_id:int,db:Session=Depends(get_db)):
    ...
    if not campaign or not _public_project(db,campaign):raise HTTPException(404,"Bag not found")
...
def enroll(campaign_id:int,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    if not campaign or campaign.status!="LIVE" or not _public_project(db,campaign):raise HTTPException(404,"Bag is not live")
    ...
    enrollment=Enrollment(user_id=user.id,campaign_id=campaign_id);db.add(enrollment);db.commit();db.refresh(enrollment);return {"ok":True,"enrollment_id":enrollment.id,"status":enrollment.status}
...
def complete_mission(campaign_id:int,mission_id:int,data:MissionCompleteIn,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    if not campaign or not _public_project(db,campaign):raise HTTPException(404,"Bag is not live")
    ...
    db.add(MissionCompletion(user_id=user.id,mission_id=mission_id,answer=data.answer,verified=verified));enrollment.completed_count+=1;user.xp+=mission.xp_reward;user.bag_score=min(1000,user.bag_score+max(1,mission.xp_reward//10));completed_now=False
    if will_complete:
        completed_now=True;enrollment.status="COMPLETED";enrollment.completed_at=datetime.now(UTC);user_amount=gross*Decimal(campaign.user_share_pct)/Decimal("100");platform_amount=gross*Decimal(campaign.nubagz_share_pct)/Decimal("100");referral_amount=gross*Decimal(campaign.referral_share_pct)/Decimal("100");enrollment.earned_amount=user_amount
        db.add(LedgerEntry(user_id=user.id,campaign_id=campaign.id,asset_symbol=campaign.reward_asset,amount=user_amount,entry_type="CAMPAIGN_REWARD",note=f"Completed {campaign.title}"));db.add(LedgerEntry(user_id=None,campaign_id=campaign.id,asset_symbol=campaign.reward_asset,amount=platform_amount,entry_type="PLATFORM_SHARE",note="NuBagz campaign share"))
...
def progress(campaign_id:int,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    enrollment=db.query(Enrollment).filter(Enrollment.user_id==user.id,Enrollment.campaign_id==campaign_id).first();ids=[r[0] for r in db.query(MissionCompletion.mission_id).join(Mission,Mission.id==MissionCompletion.mission_id).filter(MissionCompletion.user_id==user.id,Mission.campaign_id==campaign_id).all()]
