...
def save_from_campaign(data:SaveTemplateIn,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    blueprint=[{"title":m.title,"description":m.description,"mission_type":m.mission_type,"verification_type":m.verification_type,"target_url":m.target_url,"quiz_question":m.quiz_question,"quiz_options":m.quiz_options,"quiz_answer":m.quiz_answer,"xp_reward":m.xp_reward} for m in missions]
...
def instantiate(template_id:int,data:InstantiateIn,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    if project.status!="APPROVED": raise HTTPException(400,"Project must be approved before a campaign can be created")
    raw_missions=json.loads(template.mission_blueprint);mission_inputs=[MissionCreate(**mission) for mission in raw_missions];max_users=data.max_users or template.default_max_users
    ...
    for idx,mission_data in enumerate(validated.missions): db.add(Mission(campaign_id=campaign.id,position=idx,**mission_data.model_dump()))
