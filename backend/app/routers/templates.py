...
def save_from_campaign(data:SaveTemplateIn,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    for mission in missions:
        item={"title":mission.title,"description":mission.description,"mission_type":mission.mission_type,"verification_type":mission.verification_type,"target_url":mission.target_url,"quiz_question":mission.quiz_question,"quiz_options":mission.quiz_options,"quiz_answer":mission.quiz_answer,"xp_reward":mission.xp_reward}
        rule=db.query(OnchainRule).filter(OnchainRule.mission_id==mission.id).first()
        if rule:
            item["onchain_rule"]={"chain":rule.chain,"rule_type":rule.rule_type,"contract_address":rule.contract_address,"min_amount":str(rule.min_amount) if rule.min_amount is not None else None,"token_decimals":rule.token_decimals}
