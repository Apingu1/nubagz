...
def evidence_payload(evidence: ProjectTrustEvidence | None):
    if not evidence:
        return {
            "status": "NOT_SUBMITTED", "contract_address": None, "token_launch_date": None,
            "docs_url": None, "socials_url": None, "verified_at": None,
        }
    return {
        "id": evidence.id,
...
def project_trust(project: Project, db: Session):
    ...
    approval_points = 10 if project.status == "APPROVED" else 0
    ...
    if project.website:
        transparency_points += 4
    if project.treasury_address:
        transparency_points += 3
    if evidence_verified and evidence and evidence.docs_verified and evidence.socials_verified:
        transparency_points += 3
    ...
    maturity_points = min(5, project_age_days // 30) + min(5, token_age_days // 90)
...
def verify_evidence(project_id: int, data: TrustVerifyIn, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    if status not in {"VERIFIED", "REJECTED"}:
        raise HTTPException(400, "Evidence status must be VERIFIED or REJECTED")
    ...
    if not evidence or not project:
        raise HTTPException(404, "Project trust evidence not found")
    ...
    db.commit()
    db.refresh(evidence)
    return {"project_id": project_id, "evidence": evidence_payload(evidence), "trust": project_trust(project, db)}
