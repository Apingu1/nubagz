...
def seed_demo(db: Session):
    ...
    # Demo projects use the current publication state. They are LIVE, not
    # "approved" or endorsed by NuBagz; Project Trust communicates risk signals.
    projects = [
    ...
    for idx, (project, title, description, category, asset, allocation, gross, max_users, value, featured) in enumerate(specs):
        ...
        # These legacy Mission rows remain only as backwards-compatible demo data
        # for the pre-unified Bag flows. All new creator work is stored as Challenge.
        missions = [
            ...
            Mission(campaign_id=c.id, title="Pass the signal check", description="Answer one simple question to show you understood the activity.", mission_type="QUIZ", verification_type="QUIZ", quiz_question=f"Which token powers the {project.name} Bag?", quiz_options=[asset, "BTC", "USDT", "NONE"], quiz_answer=asset, xp_reward=90, position=1),
