...
function PrivySocialButtons({mode,referral,onError}:{mode:'login'|'register';referral?:string;onError:(message:string)=>void}){
  ...
  return <div className="social-auth"><div className="social-auth-label"><span>or continue with</span></div><div className="social-auth-grid social-auth-grid-two">
  ...
  </div><small className="social-auth-note">Link X once and NuBagz can use that verified identity for supported X Bag Work.</small></div>
}
