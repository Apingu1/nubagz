...
export default function BagCard({campaign,index=0}:{campaign:Campaign;index?:number}){
 ...
 return <Link to={`/app/bagz/${campaign.id}`} className="bag-card">
   ...
   <div className="card-meta"><span><Clock3 size={14}/>{campaign.missions.length*2+2} min</span><span><Users size={14}/>{remaining.toLocaleString()} spots</span><span className="go">Open Bag <ArrowUpRight size={15}/></span></div></div>
 </Link>
}
