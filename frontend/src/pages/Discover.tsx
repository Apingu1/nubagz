...
export default function Discover(){const [items,setItems]=useState<Campaign[]>([]);const [cat,setCat]=useState('ALL');const [q,setQ]=useState('');const [loading,setLoading]=useState(true)
 ...
 const filtered=useMemo(()=>items.filter(c=>(cat==='ALL'||c.category===cat)&&(!q||`${c.title} ${c.description} ${c.project?.name} ${c.reward_asset}`.toLowerCase().includes(q.toLowerCase()))),[items,cat,q])
 return <div className="page"><div className="page-head marketplace"><div><span className="eyebrow small">THE BAG MARKET</span><h1>Find your <em>next Bag.</em></h1><p>Every reward shown here is backed by a project allocation — not by your deposit.</p></div></div>
 <div className="market-toolbar"><div className="search-box"><Search/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search projects, tokens or missions…"/></div><button className="filter-button"><SlidersHorizontal/> Filters</button></div>
