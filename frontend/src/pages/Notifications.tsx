...
export default function Notifications(){
 ...
 return <div className="page"><div className="page-head"><div><span className="eyebrow small">NOTIFICATIONS</span><h1>Your NuBagz <em>signal feed.</em></h1><p>Private account events derived from real rewards, approvals and review outcomes. Read state is persisted per account, and no cross-account activity is exposed here.</p></div><button className="btn primary" disabled={!d?.unread_count} onClick={readAll}><CheckCheck/> Mark all read</button></div>{msg&&<div className="form-note">{msg}</div>}{error&&<div className="form-error">{error}</div>}
