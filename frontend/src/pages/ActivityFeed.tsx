...
const LABELS:Record<string,string>={BAG_COMPLETED:'Bag completions',BAGDROP_CLAIMED:'BagDrops'}
...
export default function ActivityFeed(){const [d,setD]=useState<Payload|null>(null);const [filter,setFilter]=useState('');const load=(type=filter)=>api<Payload>(`/activity?limit=60${type?`&event_type=${type}`:''}`).then(setD);useEffect(()=>{load('')},[])
 ...
 return <div className="page"><div className="page-head"><div><span className="eyebrow small">COMMUNITY ACTIVITY</span><h1>See what people are <em>actually doing.</em></h1><p>Real verified Bag completions and funded BagDrop claims—without exposing private wallets or balances.</p></div></div>
 ...
 <section className="panel"><div className="panel-head"><div><span>LIVE SIGNALS</span><h2>Recent participation</h2></div></div>{d?.events.length?d.events.map(e=><div className="activity-row" key={e.event_id}><div><span>{e.event_type.replaceAll('_',' ')} • {new Date(e.occurred_at).toLocaleString()}</span><strong>{e.headline}</strong><small>{e.detail}</small></div><b>{e.project_name}</b><Link className="mini-action" to={e.link_path}>Open <ArrowRight/></Link></div>):<div className="empty-state"><Activity/><strong>No community events here yet.</strong><p>Verified funded participation matching this filter will appear here.</p></div>}</section>
