...
import HumanVerification, { turnstileConfigured } from '../components/HumanVerification'
...
export default function Auth({mode}:{mode:'login'|'register'}){
 const {login,register}=useAuth(); const nav=useNavigate(); const [params]=useSearchParams(); const [form,setForm]=useState({email:'',username:'',password:'',referral:params.get('ref')||''}); const [error,setError]=useState(''); const [busy,setBusy]=useState(false);const [refStatus,setRefStatus]=useState<ReferralStatus|null>(null);const [humanRequired,setHumanRequired]=useState(false);const [humanToken,setHumanToken]=useState<string|null>(null)
 ...
 const handleHumanToken=useCallback((token:string|null)=>{setHumanToken(token);if(token)setError('')},[])
 const submit=async(e:React.FormEvent)=>{e.preventDefault();setBusy(true);setError('');try{if(mode==='login'){await login(form.email,form.password,humanToken||undefined);nav('/app')}else{await register(form.email,form.username,form.password,form.referral,humanToken||undefined);nav('/wallet-setup')}}catch(e:any){if(e instanceof ApiError&&e.status===429){setHumanRequired(Boolean(e.humanVerificationRequired));setHumanToken(null);const seconds=e.retryAfter?` Retry after about ${e.retryAfter} second${e.retryAfter===1?'':'s'}.`:'';setError(`${e.message}${seconds}`)}else setError(e.message||'Something went wrong')}finally{setBusy(false)}}
 const needsHumanToken=humanRequired&&turnstileConfigured()&&!humanToken
 return <div className="auth-page"><div className="auth-art"><Link to="/" className="back"><ArrowLeft/> Back</Link><Logo/><div className="auth-message"><span>WELCOME TO THE BAG ECONOMY</span><h1>{mode==='login'?'Your Bag is waiting.':'Start at £0. Go anywhere.'}</h1><p>Discover funded opportunities, build reputation and earn your way into crypto without putting your own money on the line.</p><div className="auth-points"><div><CheckCircle2/> No deposit to start</div><div><CheckCircle2/> Transparent funded rewards</div><div><CheckCircle2/> Your wallet, your choice</div></div></div><div className="bag-z-auth-stage"><div className="bag-z-auth-label"><span>MEET YOUR GUIDE</span><strong>Bag Z</strong></div><BagZMascot variant="hello" className="bag-z-auth-art" eager/></div></div>
 ...
 <SocialLoginButtons mode={mode} referral={form.referral} onError={setError}/>
 <div className="social-password-divider"><span>or use email & password</span></div>
 ...
 {humanRequired&&<HumanVerification onToken={handleHumanToken}/>} {error&&<div className="form-error">{error}</div>}<button className="btn primary full big" disabled={busy||needsHumanToken}>{busy?'Loading…':humanRequired&&turnstileConfigured()&&!humanToken?'Complete human check':mode==='login'?'Enter NuBagz':'Create my Bag'} <ArrowRight/></button>
