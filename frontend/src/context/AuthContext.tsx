...
type AuthContextType = {
  ...
  login:(email:string,password:string,humanToken?:string)=>Promise<void>
  register:(email:string,username:string,password:string,referral?:string,humanToken?:string)=>Promise<void>
  socialLogin:(identityToken:string,referral?:string)=>Promise<void>
  logout:()=>void
...
function humanHeaders(humanToken?:string){return humanToken?{'X-NuBagz-Human-Token':humanToken}:undefined}
...
export function AuthProvider({children}:{children:ReactNode}){
  ...
  const login=async(email:string,password:string,humanToken?:string)=>{ const r=await api<{access_token:string;user:User}>('/auth/login',{method:'POST',headers:humanHeaders(humanToken),body:JSON.stringify({email,password})});storeSession(r,'password') }
  const register=async(email:string,username:string,password:string,referral_code?:string,humanToken?:string)=>{ const r=await api<{access_token:string;user:User}>('/auth/register',{method:'POST',headers:humanHeaders(humanToken),body:JSON.stringify({email,username,password,referral_code:referral_code||null})});storeSession(r,'password') }
